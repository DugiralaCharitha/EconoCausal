...
def test_causal_graph_builder_metadata():
    """Test DOT graph string creation and metadata structure for UI."""
...
def test_dowhy_causal_model_creation():
    ...
    df = generate_retail_data(n_samples=200, seed=123)
    model, metadata = create_retail_causal_graph(df)

    assert model is not None
    # Identify estimand using DoWhy backdoor criterion
    identified_estimand = model.identify_effect(proceed_when_unidentifiable=True)
    assert identified_estimand is not None
    assert "backdoor" in identified_estimand.estimands
